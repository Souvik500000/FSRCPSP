...
def main():
    ...
    for file in folder.iterdir():
        ...
        if file.is_file() and not file.name.startswith('.') and not file.name.startswith('results'):
            ...
            if not file.name.startswith('.') and file_name_without_extension == "j102_2":
...
def fix_start_times_from_file(x, es_ls, file_path):
    with open(file_path, 'r') as file:
        start_times_data = eval(file.read())
    print("Validating fixed start times against ES-LS:")
    for (i, t) in start_times_data:
        es, ls = es_ls[i][0], es_ls[i][1]
        print(f"Activity {i}: Fixed Start={t}, ES={es}, LS={ls}")
        if not (es <= t <= ls):
            raise ValueError(f"Activity {i} start time {t} violates ES-LS [{es}, {ls}].")
        x[i][t].lb = x[i][t].ub = 1
# # Example usage:
...
def schedule_activities_fn(total_activities_count, unified_preced_array,
    ...
    #   model = Model()
      ...
      # With this:
      model = Model(solver_name='cbc')  # Explicitly use the CBC solver
      model.rel_gap = 0.01
      model.emphasis = 1  # Focus on feasibility
      model.max_seconds = 600  # Extend time limit to 10 minutes
      ...
      # Add after defining x:
    #   model += x[n+1][total_time_horizon_time_units_value - 1] == 1  # Force last activity to finish at horizon
    ...
    #   fix_start_times_from_file(x, file_path)
      # With:
      fix_start_times_from_file(x, es_ls_combined_list, file_path)  # Pass ES-LS data
    ...
    #           model += lhs >= rhs
     # Constraint 3: Precedence with linear expression
      for (j, s) in unified_preced_array:
          for p in P:
              start_time_j = xsum(t * x[j][t] for t in T)
              start_time_s = xsum(t * x[s][t] for t in T)
              model += start_time_s >= start_time_j + d_pi_i[p][j] # Use variable d_pi_i[p][j], not its value)
      # #constraint 4
      for p in P:
          for k in K:
              for t in T:
      ...
      # Constraint 6 (Revised):
      for p in P:
          for i in J:
              for k in K:
                  for t in T:
                    # Lower bound
                      model += r_pi_ikt[p][i][k][t] >= lb_rs_allocation[i] * xsum(x[i][tau] for tau in range(t + 1))
                    # Upper bound
                      model += r_pi_ikt[p][i][k][t] <= ub_rs_allocation[i] * xsum(x[i][tau] for tau in range(t + 1))
    ...
    #   print("Makespan = {}".format(model.objective_value))
        ...
        # With:
      if status == OptimizationStatus.OPTIMAL:
        print(f"Makespan = {model.objective_value}")
      else:
