...
def main():
    ...
    completed_list = ["j301_7", "j302_2", "j302_3"]
    priority_list = ["j301_1", "j301_8", "j302_6", "j302_8", "j302_9", "j303_1", "j303_3"]
    ignored_list = []
    for file in folder.iterdir():
        print(file.name)
        if file.is_file() and not file.name.startswith('.') and not file.name.startswith('results') and file not in completed_list:
            file_name_without_extension, file_extension = os.path.splitext(file.name)
            
            
            # if file_name_without_extension in ["j104_10"]:
            if not file.name.startswith('.') and file_name_without_extension not in completed_list and file_name_without_extension in priority_list:
...
def schedule_activities_fn(total_activities_count, unified_preced_array,
     ...
     m1_val, m2_val, total_time_horizon_time_units_value, R_kt, workload_dictionary, work_load_all_scenarios_list, file_name_without_extension):
      ...
      folder_path = '/Users/bhanu/Desktop/Thesis 2.0/Fix_and_optimize_Determinstic_matched_ES/j30_FAO_Determinstic_matched_ES'
      ...
      fix_start_values_folder = '/Users/bhanu/Desktop/Thesis 2.0/Matched_Results_j30_Deterministic_ES'
